...
- **`new()` 的應用**：在 Go 1.26 之後，`new()` 可被用於 expression, specifying 與 initial value of variable，不需要特別設計 helper function
- **未完成的功能**：在 `internal/lib/liberrs` 中有定義 `liberrs.Todo` 這個 error，當功能尚未完成時可以顯式使用這個 error，方便後續實作與分析
- **工具鏈**：
    - 必須通過 `gofmt` 格式化。
